import { Injectable, HttpStatus, UnauthorizedException, BadRequestException, NotFoundException, ConflictException } from '@nestjs/common';
...
export class AuthService {
  ...
  async create(createAuthDto: CreateAuthDto, res: Response) {
    try {
      ...
      let user;
      if (existingUser) {
        user = await this.userRepositories.updateUser(existingUser.id, userData);
      ...
      res.status(existingUser ? HttpStatus.OK : HttpStatus.CREATED)
  ...
  async verify(verifyUserDto: VerifyUserDto, res: Response) {
    ...
    if (!user.verificationCodeExpireAt || !OtpUtil.isValid(user.verificationCodeExpireAt)) {
    ...
    // Store refresh token
    await this.sessionRepositories.createSession(verifiedUser.id, tokens.refreshToken);
   ...
   */
  async checkVerificationCode(checkVerificationCodeDto: VerifyUserDto, res: Response) {
   ...
   */
  async resetPassword(resetPasswordAuthDto: ResetPasswordAuthDto, res: Response) {
  ...
  async login(loginAuthDto: LoginAuthDto, res: Response) {
    try {
      ...
      await this.sessionRepositories.createSession(user.id, tokens.refreshToken);
  ...
  async refreshTokens(req: any, res: Response) {
    ...
    try {
      ...
      const session = await this.sessionRepositories.findSessionByToken(refreshToken);
      ...
      await this.sessionRepositories.updateSession(session.id, tokens.refreshToken);
   ...
   * Social login
   */
  async socialLogin(user: any, res: Response) {
